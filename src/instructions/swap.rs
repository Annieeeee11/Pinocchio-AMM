...
use pinocchio::{account_info::AccountInfo, instruction::{Seed, Signer}, program_error::ProgramError, pubkey::find_program_address, sysvars::{clock::Clock, Sysvar}, ProgramResult};
...
impl<'a> TryFrom<&'a [u8]> for SwapInstructionData {
    ...
    fn try_from(data: &'a [u8]) -> Result<Self, Self::Error> {
        if data.len() != size_of::<bool>() + size_of::<u64>() * 2 + size_of::<i64>() {
        ...
        if amount.eq(&0) || min.eq(&0) || now > expiration{
            return Err(ProgramError::InvalidInstructionData); 
        }
...
impl<'a> Swap<'a> {
    ...
    pub fn process(&mut self) -> ProgramResult {
        ...
        let config_fee  = u16::from_le_bytes(config.fee().try_into().unwrap());
