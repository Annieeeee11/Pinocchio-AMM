...
impl TryFrom<&[u8]> for InitializeInstructionData {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        ...
        const INITIALZE_DATA_LEN: usize = INITIALIZE_DATA_LEN_WITH_AUTHORITY - size_of::<[u8; 32]>();
        ...
        match data.len() {
            ...
            INITIALZE_DATA_LEN => {
                let mut raw: MaybeUninit<[u8; INITIALZE_DATA_LEN]> = MaybeUninit::uninit();
                let raw_ptr = raw.as_mut_ptr() as *mut u8;
                unsafe {
                    core::ptr::copy_nonoverlapping(data.as_ptr(), raw_ptr, INITIALZE_DATA_LEN);
                    core::ptr::write_bytes(raw_ptr.add(INITIALZE_DATA_LEN), 0, 32);
...
impl<'a> Initialize<'a> {
    ...
    pub fn process(&mut self) -> ProgramResult {
        ...
        let config_account_info = self.accounts.config;
        let mut config = Config::load_mut(config_account_info)?;
